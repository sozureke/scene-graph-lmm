...
from schemas.scene_schemas import scene_schema
...
class GenerativeModel:
    ...
    def __init__(self, API_KEY: str | None, MODEL_NAME: str, SYSTEM_MESSAGE: str | None, CACHE_DIR: str = 'data/cache/') -> None:
        """
    ...
    def setup_config(
        ...
        response_schema: Optional[Dict[str, Any]] = scene_schema,
        **kwargs
    ) -> GenerationConfig:
        """
    ...
    def prompt_preparation(self, prompt_path: str, image_paths: List[str]) -> Tuple[Optional[str], List[str]]:
        """
        ...
        try:
            ...
            return text_prompt, [str(image) for image in images]
    ...
    def generate_response(self, prompt_path: str, image_paths: List[str]) -> str:
        ...
        cached_response = Utils.load_cache_entry(cache_key, self.CACHE_DIR)
        ...
        if config:
            logging.info("[!] Model generation configuration file loaded successfully.")
        logging.info("[!] Generating response...")
        try:
            response = self.model.generate_content([text_prompt] + images, stream=False, generation_config=config)
            response.resolve()
            
            output = response.text
            Utils.save_result(output)

            logging.info("[!] Response generated successfully!")
            Utils.save_cache_entry(cache_key, response.text, self.CACHE_DIR)
